...
predict_sequential_rf <- function(test_data, original_model, retrained_model) {
  # Ensure data is sorted by Electrode_No and row number
  ...
  for (group_key in unique(test_data$Electrode_No)) {
    ...
    for (i in seq_len(nrow(group_data))) {
      ...
      input_features_no_drift <- data.frame(v_in = row$v_in, v_f = row$v_f)  # No drift
      input_features_with_drift <- data.frame(v_in = row$v_in, v_f = row$v_f, cumul_drift = row$cumul_drift)  # With drift
      
      # Predict cl_conc using the no-drift model
      predicted_no_drift <- predict(original_model, input_features_no_drift)
      
      # Predict cl_conc using the model with drift, or fallback to no-drift prediction if cumul_drift = 0
      if (row$cumul_drift == 0) {
        predicted_with_drift <- predicted_no_drift
      } else {
        predicted_with_drift <- predict(retrained_model, input_features_with_drift)
      }
      
      # Store predictions
      group_data$predicted_cl_conc_no_drift[i] <- predicted_no_drift
      group_data$predicted_cl_conc_updated[i] <- predicted_with_drift
      
      # Calculate drift for the next row
      if (i < nrow(group_data)) {
        drift_value <- predicted_with_drift * 600 * 0.1  # Assuming 0.1 is the time interval
        group_data$drift[i + 1] <- drift_value
        group_data$cumul_drift[i + 1] <- group_data$cumul_drift[i] + drift_value
      }
    }
